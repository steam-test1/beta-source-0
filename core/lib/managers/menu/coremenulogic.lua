...
function Logic:init( menu_data )	
	...
	-- All the callbacks we support (demand?)
	self._callback_map = {}
	self._callback_map[ "renderer_show_node" ]			= nil -- Tell renderer to show the node
	self._callback_map[ "renderer_refresh_node" ]		= nil -- Tell renderer to update the node
	self._callback_map[ "renderer_update_node" ]		= nil -- Tell renderer to update the node
	self._callback_map[ "renderer_select_item" ]		= nil -- Tell renderer that the item has been selected
	self._callback_map[ "renderer_deselect_item" ]		= nil -- Tell renderer that the item has been deselected
	self._callback_map[ "renderer_trigger_item" ]		= nil -- Tell renderer that the item has been triggered
	self._callback_map[ "renderer_navigate_back" ]		= nil -- Tell renderer that the menu are navigating back
	self._callback_map[ "renderer_node_item_dirty" ]	= nil -- Tell renderer that the item in the node is dirty
	self._callback_map[ "input_accept_input" ]			= nil -- Tell input to accept( bool ) input 
	self._callback_map[ "menu_manager_menu_closed" ]	= nil -- Tell the menu manager that the menu is closed (renderer has finished animating, etc) 
...
function Logic:open(...)
	...
	-- self:_select_node( nil )
end

function Logic:_queue_action( action_name, ... )
	table.insert( self._action_queue, { action_name = action_name, parameters = { ... } } )
end
...
function Logic:_select_node( node_name, ... )
	...
	if has_active_menu and node then
		...
		if node:parameters().menu_components then
			...
			-- print( inspect( node:parameters().menu_components ) )
		end
...
function Logic:_update_node( node_name, ... )
	...
	if node then	
		-- table.insert( self._node_stack, node )
		self:_call_callback( "renderer_update_node", node )
...
function Logic:navigate_back( queue, skip_nodes )		-- skip_nodes=how many nodes to skip, 0=no nodes are skiped, normal back functionality
	if self._accept_input or queue then
...
function Logic:_navigate_back( skip_nodes )
	...
	if node then
		if node:trigger_back() then	-- block back if trigger_back() returns true
			-- Application:debug( "CoreLogic:_navigate_back(): Back is blocked!" )
			return
		end
...
-- Called when it gets into focus again (after another menu has been closed)
...
function Logic:close( closing_menu )
	...
	--[[if selected_node then
		if node:parameters().menu_components then
			managers.menu_component:set_active_components( node:parameters().menu_components )
		end
	end]]
