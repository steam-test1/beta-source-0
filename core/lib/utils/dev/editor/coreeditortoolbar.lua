function CoreEditor:build_toolbar()
	...
	Global.frame:connect( "TB WIDGET SELECT", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "set_widget" ), { select = true, move = false, rotate = false } )
	self._toolbar:add_radio_tool( "TB WIDGET MOVE", "Select and Move", icons_path.."widget_move.bmp", "Select and Move" )
	Global.frame:connect( "TB WIDGET MOVE", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "set_widget" ), { select = false, move = true, rotate = false } )
	...
	Global.frame:connect( "TB WIDGET ROTATE", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "set_widget" ), { select = false, move = false, rotate = true } )
	...
	self._toolbar:connect( "TB_SURFACE_MOVE", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toolbar_toggle" ), { value = "_use_surface_move", menu = "_edit_menu" } )
	self._ews_triggers[ "surface_move_toggle" ] = callback( self, self, "toolbar_toggle_trg", { value = "_use_surface_move", id = "TB_SURFACE_MOVE", menu = "_edit_menu" } )
	...
	self._toolbar:connect( "TB_SNAPPOINTS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toolbar_toggle" ), { value = "_use_snappoints", menu = "_edit_menu" } )
	self._ews_triggers[ "use_snappoints_toggle" ] = callback( self, self, "toolbar_toggle_trg", { value = "_use_snappoints", id = "TB_SNAPPOINTS", menu = "_edit_menu" } )
	...
	self._ews_triggers[ "using_group_toggle" ] = callback( self, self, "toolbar_toggle_trg", { value = "_using_groups", id = "TB_USING_GROUPS" } )
	...
	self._toolbar:connect( "TB_LAYER_DRAW_GRID", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toolbar_toggle" ), { value = "_layer_draw_grid", toolbar = "_toolbar" } )
	...
	self._ews_triggers[ "change_snaprot_axis" ] = callback( self, self, "change_snaprot_axis", { value = "_coordinate_system", combobox = self._ref_coordinate_system, t = "_coordinate_systems", menu = self._coordinate_menu } )
	...
	-- self._toolbar:set_tool_separation( 5 )
	-- self._toolbar:set_tool_packing( 5 )
...
function CoreEditor:build_ref_coordinate_system()
	...
	for _,coor in ipairs( self._coordinate_systems ) do
		...
		Global.frame:connect( coor, "EVT_COMMAND_MENU_SELECTED", callback( self, self, "set_combobox_value" ), { value = "_coordinate_system", combobox = self._ref_coordinate_system, menu = self._coordinate_menu } )
	end
	...
	self._ref_coordinate_system:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "change_combo_box" ), { value = "_coordinate_system", combobox = self._ref_coordinate_system, menu = self._coordinate_menu } )
		
	self._ews_triggers[ "toggle_coordinate_system" ] = callback( self, self, "change_combo_box_trg", { value = "_coordinate_system", combobox = self._ref_coordinate_system, t = "_coordinate_systems", menu = self._coordinate_menu } )
end
...
--	grid_icon:set_size( Vector3( 10, -1, 0 ) )
	...
	for _,g in pairs( self._grid_sizes ) do
		sizes:append( g )
		Global.frame:connect( "TB_GRIDSIZE"..g, "EVT_COMMAND_MENU_SELECTED", callback( self, self, "set_combobox_value" ), { value = "_grid_size", combobox = sizes, menu = self._grid_sizes_menu, choice = g } )
	end
	...
	sizes:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "change_combo_box" ), { value = "_grid_size", combobox = sizes, menu = self._grid_sizes_menu, prefix = "TB_GRIDSIZE" } )
	
	self._ews_triggers[ "change_grid_size" ] = callback( self, self, "change_combo_box_trg", { value = "_grid_size", combobox = sizes, t = "_grid_sizes", menu = self._grid_sizes_menu, prefix = "TB_GRIDSIZE" } )
end
...
--	snap_icon:set_size( Vector3( 10, -1, 0 ) )
	...
	for _,r in pairs( self._snap_rotations ) do
		rotations:append( r )
		Global.frame:connect( "TB_SNAPROTATION"..r, "EVT_COMMAND_MENU_SELECTED", callback( self, self, "set_combobox_value" ), { value = "_snap_rotation", combobox = rotations, menu = self._snap_rotations_menu, choice = r } )
	end
	...
	rotations:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "change_combo_box" ), { value = "_snap_rotation", combobox = rotations, menu = self._snap_rotations_menu, prefix = "TB_SNAPROTATION" } )
	
	self._ews_triggers[ "change_snaprot" ] = callback( self, self, "change_combo_box_trg", { value = "_snap_rotation", combobox = rotations, t = "_snap_rotations", menu = self._snap_rotations_menu, prefix = "TB_SNAPROTATION" } )
end
...
--	speed_icon:set_size( Vector3( 10, -1, 0 ) )
...
-- Updates the rotation speed from the spin ctrlr
function CoreEditor:update_rot_speed( rotation_speed )
	self._rotation_speed = rotation_speed:get_value()
end

-- Updates the rotation speed through a short cut key trigger
...
function CoreEditor:change_combo_box( data )
	...
	if data.menu then
		...
		if data.prefix then id = data.prefix..id end
...
-- Displays a dialog containing all hidden units
function CoreEditor:on_unhide_by_name()
	self:show_dialog( "unhide_by_name", "UnhideByName" )
end

-- Creates the icons for the global widgets (select, move and rotate)
-- It also adds ipnput shortcuts to the self._ews_triggers table
