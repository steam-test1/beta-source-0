...
function MissionLayer:init( owner )
	...
	self._name_brush:set_color( Color( 1.0, 1, 1, 1 ) )
...
function MissionLayer:load( world_holder, offset )
	...
	-- Make sure that old scripts gets a continent property
...
function MissionLayer:condition()
	return MissionLayer.super.condition( self ) or self._editing_mission_element -- or self:adding_to_mission()
end
...
function MissionLayer:set_select_unit( unit, ... )
	if alive( unit ) and unit:mission_element_data().script then
...
function MissionLayer:_add_on_executed( unit )
	if not alive( unit ) then
		if self:adding_to_mission() then -- Do not deselect selected unit if we are trying to add to on executed but misses unit
			return true
		end
		return false
	end
		
	if self._selected_unit and self:adding_to_mission() then
...
-- Hides all the unit in the layer
function MissionLayer:hide_all()
	for _,unit in ipairs( self._created_units ) do
		-- For each unit in the script...
		if unit:mission_element_data().script == self:current_script() then
...
function MissionLayer:set_enabled( enabled ) -- Since this is called from end simulation when only should enable the mission we simulated
...
function MissionLayer:use_widget_position( pos )
	if self._editing_mission_element then
		if alive( self._selected_unit ) then
			if self._selected_unit:mission_element():use_widget_position( pos ) then
				return
			end
		end
	end
...
-- Returns a pairs table for the units, the key is the unit id
...
function MissionLayer:update( time, rel_time )
	...
	for _,unit in ipairs( self._created_units ) do
		if unit:mission_element_data().script == current_script 
			  and not current_continent_locked
				or self._show_all_scripts then
			...
			if ( update_selected or selected_unit ) then
				...
				if self._override_lod_draw or distance < lod_draw_distance then
			...
			if distance < 1500 * 1500 then
				local a = distance < 1000 * 1000 and 1 or ((1500 * 1500 - distance)/(500*500))/5
	...
	-- if self._selected_unit then
		-- self._selected_unit:mission_element():update_selected( time, rel_time )
		-- self._selected_unit:mission_element():draw_connections_selected( time, rel_time )
		
	--	if self._editing_mission_element then
	--		self._selected_unit:mission_element():update_editing( time, rel_time, self._current_pos )
	--	end
		
		-- self._name_brush:set_color( Color( 1, 0, 1, 0 ) )
		-- self._name_brush:center_text( self._selected_unit:position() + Vector3( 0, 0, 200 ), self._selected_unit:unit_data().name_id )
	-- end
end
...
function MissionLayer:set_current_panel_visible( visible )
	if self._current_panel then
		-- Layer creates panel don't have extension, only unit created has it.
		if not self._current_panel:extension() or ( self._current_panel:extension() and self._current_panel:extension().alive ) then
...
function MissionLayer:toolbar_toggle_trg( data )
	if data.value == "_editing_mission_element" then -- Prevent the effect of the callback trigger to occur
		if not alive( self._selected_unit ) or not self._selected_unit:mission_element()[ "update_editing" ] then
			return
		end
	end
...
-- Returns the panel that mission elements who uses their own gui should use as parent
function MissionLayer:missionelement_panel()
	return self._missionelement_panel
end

-- Returns the sizer that mission elements who to uses their own gui should add their panel
...
function MissionLayer:build_panel( notebook )
	...
	local btn_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		self._element_toolbar:set_tool_state( "EDIT_ELEMENT", self._editing_mission_element )
		self._element_toolbar:connect( "EDIT_ELEMENT", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toolbar_toggle" ), { class = self, value = "_editing_mission_element", menu = nil, toolbar = "_element_toolbar" } )

		self._ews_triggers[ "insert" ] = callback( self, self, "toolbar_toggle_trg", { class = self, value = "_editing_mission_element", menu = nil, toolbar = "_element_toolbar", id = "EDIT_ELEMENT" } )
...
function MissionLayer:_build_scripts()
	local sizer = EWS:StaticBoxSizer( self._ews_panel, "HORIZONTAL", "Scripts" )
		...
		self._scripts_right_toolbar = EWS:ToolBar( self._ews_panel, "", "TB_FLAT,TB_NODIVIDER" )
			...
			self._scripts_right_toolbar:connect( "SIMULATE_WITH_CURRENT_SCRIPT", "EVT_COMMAND_MENU_SELECTED", callback( nil, CoreEditorUtils, "toolbar_toggle" ), { class = self, value = "_simulate_with_current_script", menu = nil, toolbar = "_scripts_right_toolbar" } )
...
function MissionLayer:add_btns_to_toolbar( ... )
	...
	self._btn_toolbar:connect( "DRAW_SELECTED_CONNECTIONS_ONLY", "EVT_COMMAND_MENU_SELECTED", callback( nil, CoreEditorUtils, "toolbar_toggle" ), { class = self, value = "_only_draw_selected_connections", menu = nil, toolbar = "_btn_toolbar" } )
	...
	self._btn_toolbar:connect( "UPDATE_SELECTED_ALL", "EVT_COMMAND_MENU_SELECTED", callback( nil, CoreEditorUtils, "toolbar_toggle" ), { class = self, value = "_update_all", menu = nil, toolbar = "_btn_toolbar" } )
	...
	self._btn_toolbar:connect( "VISUALIZE_FLOW", "EVT_COMMAND_MENU_SELECTED", callback( nil, CoreEditorUtils, "toolbar_toggle" ), { class = self, value = "_visualize_flow", menu = nil, toolbar = "_btn_toolbar" } )
	...
	self._btn_toolbar:connect( "USE_COLORED_LINKS", "EVT_COMMAND_MENU_SELECTED", callback( nil, CoreEditorUtils, "toolbar_toggle" ), { class = self, value = "_use_colored_links", menu = nil, toolbar = "_btn_toolbar" } )
end
...
-- If name is provide and it exists, use it
-- Else find a script in the current continent
-- Else set a script from any continent
function MissionLayer:_set_scripts_combobox( name )
	name = nil or self._scripts[ name ] and name
...
-- Returns a name of a script, either in a continent or any
...
function MissionLayer:_reset_scripts()
	...
	-- self:_populate_scripts_combobox( self._default_script_name )
	-- self:_populate_scripts_combobox()
end
...
-- Takes name and new name to rename the script
...
--	if managers.editor:continent( self._scripts[ name ].continent ):value( "locked" ) then
--		return
--	end
	for _,unit in ipairs( self._created_units ) do
		if unit:mission_element_data().script == name then
