...
function StaticLayer:init( owner, save_name, units_vector, slot_mask)
	...
	self._move_unit_rep = CoreInput.RepKey:new( { "move_forward", "move_back", "move_right", "move_left", "move_up", "move_down" } )
...
function StaticLayer:clone()
	...
	if self._selected_unit and not self:condition() then
		...
		for _,unit in ipairs( clone_units ) do
			...
			-- do_spawn_unit will generate a name id based on the unit name. When cloning the name id should be based on the cloned
			-- units name id, therefor it needs to be removed from register and then make a call to get a new one.
			...
			-- Clone will go through any edited properties of the unit and set them to the clone
...
-- Will also be called from GroupHanlder when cloning a group and it expects that the spawned unit is returned.
...
function StaticLayer:set_unit_positions( pos )
	...
	-- Set the reference unit position after the others have been updated. Otherwise a unit on same position
	-- as reference unit will be offsetet when set_locals is called
...
function StaticLayer:position_as()
	if self._selected_unit and not self:condition() then
		local data = { mask = self._position_as_slot_mask, ray_type = "body editor", sample = true }
...
function StaticLayer:set_select_unit( unit )
	...
	if alive( self._selected_unit ) then 
		managers.editor:set_grid_altitude( self._selected_unit:position().z ) -- The altitude should always be taken from the reference unit(selected_unit)
	end
end
...
function StaticLayer:update( t, dt )
	...
	if not self:condition() then
		if self._grab and ( self:shift() and not managers.editor:invert_move_shift() ) 
			or ( not self:shift() and managers.editor:invert_move_shift() ) then
			self._offset_move_vec = Vector3( 0, 0 ,0 )
		end
...
function StaticLayer:draw_units( t, dt )
	-- local unit_brush = Draw:brush( Color( 0.15, 1, 1, 1 )  )
	...
	-- unit_brush:unit( self._selected_unit )
end
