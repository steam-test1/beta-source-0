-- Creates a modal dialog from a list of strings. The dialog can be filtered. 
-- When dialog is closed, function selected_assets() can be called to get the result. 
...
function SelectNameModal:init( name, assets_list, ... )
	...
	-- SelectNameModal.init( self, name, unit_filter_function, ... )
	self:show_modal()
end
...
function SelectNameModal:_selected_item_assets()
	...
	for _,i in ipairs( self._list:selected_items() ) do
		...
		--local asset = self._assets_list[ i+1 ]
