...
-- A generic class that can be used for different kinds of editor dialogs
CoreEditorEwsDialog = CoreEditorEwsDialog or mixin(class(), BasicEventHandling)

--CoreEditorEwsDialog( EWSWindow parent, string caption, string id, Vector3 position, Vector3 size, string style )
...
--	local visible = settings and settings.visible or true
...
-- Displays a ListCtrlr containing all units based on unit name.
-- Clicking on unit will disply all instances off that unit in the level base on name id
...
--		unit_stats:puts( "Name,Amount,Geometry Memory,Models,Author,Unit Filename,Object filename,Diesel Filename,Material Filename,Last Exported From" )
		...
		table.insert( self._column_states, { value = "nr_textures", state = "random" } )
		table.insert( self._column_states, { value = "nr_materials", state = "random" } )
		table.insert( self._column_states, { value = "vertices_per_tris", state = "random" } )
		table.insert( self._column_states, { value = "instanced", state = "random" } )
		table.insert( self._column_states, { value = "author", state = "random" } )
		table.insert( self._column_states, { value = "unit_filename", state = "random" } )
		table.insert( self._column_states, { value = "model_filename", state = "random" } )
		table.insert( self._column_states, { value = "diesel_filename", state = "random" } )
		table.insert( self._column_states, { value = "material_filename", state = "random" } )
		table.insert( self._column_states, { value = "last_exported_from", state = "random" } )
...
--[[
function UnitList:on_delete()
	if self._unit_list:selected_item() ~= -1 then
		self:on_select_unit_list_unit()
...
-- Ignore unit is used when called from deleted_unit since the unit isn't really deleted yet
function UnitList:on_select_unit_list( ignore_unit )
	...
	if index ~= -1 then
		...
		for _,unit in ipairs( units ) do
			if unit:name():s() == unit_name then
				if unit:unit_data() and not ( ignore_unit and ignore_unit == unit ) then
...
function UnitList:deleted_unit( unit )
	...
	for i = 0, self._list:item_count()-1 do
		if self._list:get_item( i, 0 ) == unit:name():s() then
			...
			if index ~= -1 then
				-- The deleted unit is visible in the unit list
				if index == i then
					...
					--[[
					for _,i in ipairs( self._unit_list:selected_items() ) do
						local unit = self._unit_list_units[ i+1 ]
						table.insert( units, unit )
					end
					]]
				--[[
					for i,u in ipairs( self._unit_list_units ) do
						if unit == u then
							local j = self._unit_list:delete_item( i-1 )
							-- table.remove( self._unit_list_units, j )
							break
						end
					end
				]]
...
function UnitList:fill_unit_list()
	...
	for name,t in pairs( data ) do
		unique = unique + 1 
		-- self:append_item( name, t.amount, t.memory, t.models, t.nr_bodies, t.author )
		self:append_item( name, t )
--			unit_stats:puts( name..','..t.amount..','..t.memory..','..t.models..','..t.author..','..t.unit_filename..','..t.model_filename..','..t.diesel_filename..','..t.material_filename..','..t.last_exported_from )
	end
...
function UnitList:append_item( name, t )
	...
	-- self._list:set_item_data_ref( i, { name = name, amount = amount, memory = memory, models = models, bodies = nr_bodies, author = author } )
end
...
-- Displayes a unit browser using a tree. Allows to browse through all placeable editor units and look at the preview image of them.
...
--		self._unit_name:set_value( name )
	end
end
...
--	for _,self._unit_tree:get_children( self._root )
end
...
-- Shows a dialog with a list over all availible editor units. Search for one specific unit can be made through a filter function.
...
function GlobalSelectUnit:init( ... )
	...
	self._all_names = self:_all_unit_names()
		...
		-- No dubble click event for ListCtrl? self._units:connect( "EVT_COMMAND_LISTBOX_DOUBLECLICKED", callback( self, self, "on_cancel" ) )
...
function GlobalSelectUnit:update_list( current )
	...
	-- for type,ctrl in pairs( self._layer_cbs ) do
		...
		if self._only_list_used_units then
			...
			for name,type in pairs( self._all_names ) do
				-- print( name,type, inspect( self._layer_cbs ) )
				if self._layer_cbs[ type ]:get_value() then
					local stripped_name = self._short_name:get_value() and self:_stripped_unit_name( name ) or name
					if string.find( stripped_name, filter, 1 ,true ) then
						local i = self._units:append_item( stripped_name )
...
function ReplaceUnit:init( name, types )
	...
	for c,names in pairs( category_map ) do
		...
		units_sizer:add(units, 1, 0, "EXPAND") -- FIXED_MINSIZE
		units:connect( "EVT_COMMAND_LISTBOX_SELECTED", callback( self, self, "replace_unit_name" ), units )
				
		unit_filter:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "update_filter" ), { filter = unit_filter, units = units, names = names } )
...
--		self._units:connect( "EVT_COMMAND_LISTBOX_SELECTED", callback( self, self, "on_select_unit_dialog" ), self._units )
		self._units:connect( "EVT_COMMAND_LISTBOX_DOUBLECLICKED", callback( self, self, "replace_unit" ), { units = self._units, all = false })
		self._units:connect( "EVT_KEY_DOWN", callback( self, self, "key_cancel" ), "" )
		...
		local btn_sizer = EWS:BoxSizer("HORIZONTAL")
			...
			replace_btn:connect( "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "replace_unit" ), { units = self._units, all = false } )
			...
			replace_all_btn:connect( "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "replace_unit" ), { units = self._units, all = true } )
...
function LayerReplaceUnit:replace_unit( data )
	if self._layer:selected_unit() and data then
		...
		if i > -1 then
			...
			self._layer:replace_unit( name, data.all ) -- is in CoreLayer
		end
	end
end
...
function MoveTransformTypeIn:_create_ctrl( name, coor, value, type, sizer )
	local ctrl_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		spin:connect( "EVT_SCROLL_LINEUP", callback( self, self, "update_spin" ), { ctrl = c, coor = coor, step = 0.1 } )
		spin:connect( "EVT_SCROLL_LINEDOWN", callback( self, self, "update_spin" ), { ctrl = c, coor = coor, step = -0.1 } )
...
function CameraTransformTypeIn:init()
	-- CoreEditorEwsDialog.init( self, nil, "Camera transform type-in", "", Vector3( 761, 180, 0), Vector3( 264, 111, 0), "DEFAULT_DIALOG_STYLE,RESIZE_BORDER,STAY_ON_TOP" )
...
function CameraTransformTypeIn:_create_ctrl( name, coor, value, type, sizer )
	local ctrl_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		if type == "offset" then
			...
			spin:connect( "EVT_SCROLL_LINEUP", callback( self, self, "update_rotation_spin" ), { ctrl = ctrl, coor = coor, step = 1 } )
			spin:connect( "EVT_SCROLL_LINEDOWN", callback( self, self, "update_rotation_spin" ), { ctrl = ctrl, coor = coor, step = -1 } )
		else
			...
			spin:connect( "EVT_SCROLL_LINEUP", callback( self, self, "update_position_spin" ), { ctrl = ctrl, coor = coor, step = 10 } )
			spin:connect( "EVT_SCROLL_LINEDOWN", callback( self, self, "update_position_spin" ), { ctrl = ctrl, coor = coor, step = -10 } )
		end
...
--		self._graph_view:set_clipping( false )
		...
		for _,unit in ipairs( managers.editor._layers.Mission:created_units() ) do
			...
			-- if unit:name() == "hub" then
			-- 	node:set_colour( 0, 1, 0 )
			--end
		...
		for _,unit in ipairs( managers.editor._layers.Mission:created_units() ) do
			-- if unit:name() == "hub" then
				...
				-- for _,e_unit in ipairs( unit:mission_element()._on_executed_units ) do
				...
				--[[
				for _,t_unit in ipairs( unit:mission_element_data().triggers ) do
					local node = self._nodes[ t_unit:unit_data().unit_id ]	
					node:set_colour( 0, 1, 1 )
					-- node:set_target( node:free_slot(), node, "trigger" )
					node:set_target( node:free_slot(), node, "trigger" )
				end
				]]
			-- end
		...
		-- self._graph_view:set_autolayout( "EQUILIBRIUM" )
		...
		-- local test_panel = managers.editor._layers.HubElements:selected_unit():mission_element():panel( "graph", self._panel, self._panel_sizer )
		-- test_panel:set_visible( true )
	
	self._graph:notify_views()
				
	self._dialog_sizer:add( self._panel, 1, 0, "EXPAND" )
	
	-- self._dialog_sizer:add( self._graph_view, 1, 0, "EXPAND" )
...
function UnitDuality:init( collisions, pos )
	...
	self:create_panel( "VERTICAL" )
		...
		-- Used for adding size to the dialog when no collisions are found
		local y_size = 0
...
function UnitDuality:build_collision( collision )
	...
	panel:set_sizer( sizer )
		...
		del1:connect( "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "delete_unit" ), { unit = u1, panel = panel, text = text1 } )
		...
		del2:connect( "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "delete_unit" ), { unit = u2, panel = panel, text = text2 } )
