...
-- Called from the editor when a unit has changed its name id
	...
	for name,layer in pairs( layers ) do
		if self._layer_cbs[ name ]:get_value() then
			for _,unit in ipairs( layer:created_units() ) do
				if string.find( self:_get_filter_string( unit ), filter, 1 ,true ) then
					if self:_unit_condition( unit ) then
						local i = self._list:append_item( unit:unit_data().name_id )
						self._units[ j ] = unit
						self._list:set_item_data( i, j )
						-- self._list:set_item_background_colour( i, Vector3( 1, 0, 0 ) )
