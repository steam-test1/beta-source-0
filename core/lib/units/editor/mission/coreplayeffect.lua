...
function CorePlayEffectUnitElement:init( unit )	
	...
	--[[
	table.insert( self._hed.action_types, "spawn" )
	table.insert( self._hed.action_types, "kill" )
	table.insert( self._hed.action_types, "fade_kill" )
	]]
end
...
function CorePlayEffectUnitElement:test_element()
	if self._hed.effect ~= "none" then
		...
		self._effect = World:effect_manager():spawn( { effect = self._hed.effect:id(), position = position, rotation = rotation } )
	end
end
...
function CorePlayEffectUnitElement:_build_panel( panel, panel_sizer )
	...
	local screen_space = EWS:CheckBox( panel, "Play in Screen Space", "" )
		...
		screen_space:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = screen_space, value = "screen_space" } )
	...
	random_time:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = random_time, value = "random_time" } )
	random_time:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = random_time, value = "random_time" } )
...
function CorePlayEffectUnitElement:add_to_mission_package()
	if self._hed.effect and self._hed.effect ~= "none" then
		managers.editor:add_to_world_package( { category = "effects", name = self._hed.effect, continent = self._unit:unit_data().continent } )
	end
end
...
function CoreStopEffectUnitElement:add_element()
	...
	if ray and ray.unit and string.find( ray.unit:name():s(), "env_effect_play", 1 ,true ) then -- ray.unit:name() == "me_effect_play" then
