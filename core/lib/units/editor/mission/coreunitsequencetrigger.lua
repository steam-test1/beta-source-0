...
function CoreUnitSequenceTriggerUnitElement:init( unit )
	...
	self._hed.sequence_list = {}  -- { unit_id = unit_id, sequence = sequence }
...
function CoreUnitSequenceTriggerUnitElement:layer_finished()
	...
	if self._hed.unit_id then -- CONVERT
		table.insert( self._hed.sequence_list, { unit_id = self._hed.unit_id, sequence =  self._hed.sequence } )
	end
	
	-- self._sequence_unit = managers.worlddefinition:get_unit_on_load( self._hed.unit_id, callback( self, self, "load_unit" ) )
...
function CoreUnitSequenceTriggerUnitElement:select_unit()
	...
	if ray and ray.unit then
		...
		if #sequences > 0 then
			self._sequence_units[ unit:unit_data().unit_id ] = unit
			local sequence_list_data =  { unit_id = unit:unit_data().unit_id, sequence = "none" }
...
function CoreUnitSequenceTriggerUnitElement:_add_unit( unit, sequences, sequence_list_data )
	...
	-- self._guis[ unit:unit_data().unit_id ] = { unit_id = unit_id, sequence = sequence, name_ctrlr = sequence_params.name_ctrlr, toolbar = toolbar }
	self._guis[ self._guis_id ] = { unit_id_ctrlr = unit_id, unit = unit, unit_id = unit:unit_data().unit_id, sequence = sequence, name_ctrlr = sequence_params.name_ctrlr, toolbar = toolbar, guis_id = self._guis_id }
	
	h_sizer:add( toolbar, 0, 1, "EXPAND,LEFT" )
	
	-- sequence:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = sequence, value = "sequence" } )
